...
def my_goals_list():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute(f"""
                SELECT goal_id, day_number, completed, completion_date
...
def track_goal_day():
    ...
    if not email and not user_id_param:
        return jsonify({"status": "error", "message": "email or user_id is required"}), 400
    if not goal_id:
        return jsonify({"status": "error", "message": "goal_id is required"}), 400
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if not goal:
                return jsonify({"status": "error", "message": "Goal not found"}), 403

            # Count completed days
            cursor.execute("""
                SELECT COUNT(*) AS completed_count
                FROM goal_daily_tracking
                WHERE goal_id=%s AND completed=1
            """, (goal_id,))

            row = cursor.fetchone()
            completed_count = int(row["completed_count"] or 0)

            next_day_number = completed_count + 1

            if next_day_number > goal["total_days"]:
                return jsonify({
                    "status": "error",
                    "message": "All days already completed"
                }), 400

            # Insert next day completion
            cursor.execute("""
                INSERT INTO goal_daily_tracking
                (goal_id, day_number, completed, completion_date)
                VALUES (%s, %s, %s, CURDATE())
                ON DUPLICATE KEY UPDATE
                    completed=%s,
                    completion_date=CURDATE()
            """, (goal_id, next_day_number, completed, completed))
        ...
        return jsonify({
            "status": "success",
            "message": f"Day {next_day_number} completed",
...
def _recalculate_progress(conn, user_id):
    with conn.cursor() as cursor:
        # NEAT INTEGRATION: Count unique active days from BOTH habit logs AND goal tracking
        cursor.execute("""
